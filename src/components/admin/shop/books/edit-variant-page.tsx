...
export function EditVariantPage({ variantId }: EditVariantPageProps) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["book-variant", variantId],
    queryFn: () => getBookVariant(variantId),
  });
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try {
      const result = await updateBookVariant(formData);
      if (result.message) {
    ...
    <div className="w-full max-w-7xl mx-auto p-6">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Editing {variant.variant} version of &quot;{book.tittle}&quot;
        </p>
      </div>
