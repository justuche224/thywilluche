...
const BlogComments = ({ postId }: { postId: string }) => {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      const result = await getBlogComments({ postId, page: 1, limit: 20 });
      return result;
    },
  });
  ...
  const createCommentMutation = useMutation({
    mutationFn: async (content: string) => {
      return await createBlogComment({ postId, content });
    },
    onSuccess: (result) => {
      if (result.success) {
...
const Comment = ({
  ...
  const { data: likeStatus } = useQuery({
    queryKey: ["blog-comment-like-status", comment.id],
    queryFn: async () => checkBlogCommentLikeStatus({ commentId: comment.id }),
  });
  ...
  const likeMutation = useMutation({
    mutationFn: async () => toggleBlogCommentLike({ commentId: comment.id }),
    onMutate: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => deleteBlogComment({ commentId: comment.id }),
    onSuccess: (result) => {
      if (result.success) {
        toast.success(result.message);
        queryClient.invalidateQueries({ queryKey: ["blog-comments", postId] });
  ...
  const replyMutation = useMutation({
    mutationFn: async (content: string) => {
      return await createBlogComment({
        postId,
        content,
        parentId: comment.id,
      });
    ...
    onSuccess: (result) => {
      if (result.success) {
        ...
        setShowReplies(true);
        queryClient.invalidateQueries({
          queryKey: ["blog-comment-replies", comment.id],
        });
        queryClient.invalidateQueries({ queryKey: ["blog-comments", postId] });
      } else {
        if (result.message.includes("sign in")) {
...
const CommentReplies = ({
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["blog-comment-replies", commentId],
    queryFn: async () => {
...
const Reply = ({ reply, postId }: { reply: CommentData; postId: string }) => {
  ...
  const { data: likeStatus } = useQuery({
    queryKey: ["blog-comment-like-status", reply.id],
    queryFn: async () => checkBlogCommentLikeStatus({ commentId: reply.id }),
  });
  ...
  const likeMutation = useMutation({
    mutationFn: async () => toggleBlogCommentLike({ commentId: reply.id }),
    onMutate: async () => {
      setLocalLikeCount((prev) => (isLiked ? prev - 1 : prev + 1));
    },
    onSuccess: (result) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => deleteBlogComment({ commentId: reply.id }),
    onSuccess: (result) => {
      if (result.success) {
        toast.success(result.message);
        queryClient.invalidateQueries({ queryKey: ["blog-comment-replies"] });
        queryClient.invalidateQueries({ queryKey: ["blog-comments", postId] });
      } else {
        toast.error(result.message);
      }
  ...
  const replyMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.success) {
        ...
        queryClient.invalidateQueries({
          queryKey: ["blog-comment-replies", reply.id],
        });
        queryClient.invalidateQueries({ queryKey: ["blog-comments", postId] });
      } else {
        if (result.message.includes("sign in")) {
